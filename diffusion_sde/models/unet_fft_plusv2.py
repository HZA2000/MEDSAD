...
def create_circular_mask(h, w, center=None, radius=None):
    ...
    # Y, X = np.ogrid[:h, :w]
    Y = torch.tensor([x for x in range(h)]).reshape(-1, 1).cuda()
    X = torch.tensor([x for x in range(h)]).reshape(1, -1).cuda()
...
class Clamp(torch.autograd.Function):
    ...
    def forward(ctx, input, min_val, max_val):
        return input.clamp(min=min_val, max=max_val) # the value in iterative = 2
...
class Conditioning(nn.Module):
    def __init__(self, fmap_size, dim):
        ...
        self.inner_radius, self.outer_radius = nn.Parameter(torch.tensor(1).float()), nn.Parameter(torch.tensor(fmap_size // 2).float())
        
        # self.inner_attn_map = create_circular_mask(fmap_size, fmap_size, self.inner_radius)
        # self.outer_attn_map = create_circular_mask(fmap_size, fmap_size, self.outer_radius)
        # self.ff_parser_attn_map = torch.logical_xor(self.inner_attn_map, self.outer_attn_map)

        self.norm_input = LayerNorm(dim, bias = True)
        self.norm_condition = LayerNorm(dim, bias = True)

        self.block = ResnetBlock(dim * 2, dim)
    ...
    def forward(self, x):
        ...
        # x, c = torch.chunk(input, 2, 1)
        # dtype = x.dtype
        # self.outer_radius = torch.clip(self.outer_radius, 1, self.fmap_size // 2)
        # self.inner_radius = torch.clip(self.inner_radius, 1, self.outer_radius)
        # self.outer_radius = self.clamp_class.apply(self.outer_radius, 1, self.fmap_size // 2)
        self.outer_radius.data.clamp_(1, self.fmap_size // 2)
        self.inner_radius.data.clamp_(1, self.outer_radius.data)
        
        self.inner_attn_map = create_circular_mask(self.fmap_size, self.fmap_size, radius=self.inner_radius.item())
        self.outer_attn_map = create_circular_mask(self.fmap_size, self.fmap_size, radius=self.outer_radius.item())
        self.ff_parser_attn_map = torch.tensor(1.) - torch.logical_xor(self.inner_attn_map, self.outer_attn_map).float()
        ...
        # x = x.type(dtype)
        # print(self.outer_radius.max(), self.outer_radius.min())
        # eq 3 in paper
...
#     def __init__(self, fmap_size, dim):
#         super().__init__()
#         self.ff_parser_attn_map = nn.Parameter(torch.ones(dim // 2, fmap_size, fmap_size))

#         self.norm_input = nn.LayerNorm(dim // 2, bias = True)
#         self.norm_condition = nn.LayerNorm(dim // 2, bias = True)
...
#         print(x.shape)
#         coefficients = ptwt.wavedec2(x, pywt.Wavelet("haar"),
#                                 level=2, mode="constant")
#         print(coefficients.shape)
#         coefficients = coefficients * self.ff_parser_attn_map
#         print(coefficients.shape)
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mults):
            ...
            for _ in range(res_blocks):
                ...
                # layers.append(
                #         Conditioning(curr_fmap_size, int(mult * inner_channel))
                #     )
            ...
            if level != len(channel_mults) - 1:
                ...
                if resblock_updown:
                    ...
                    self.input_blocks.append(Conditioning(curr_fmap_size, int(mult * inner_channel)))
                    self.input_blocks.append(
                        Downsample(
                ...
                ds *= 2
                self._feature_size += ch
                curr_fmap_size = curr_fmap_size // 2
    ...
    def forward(self, x, gammas):
        ...
        # print(len(self.input_blocks))   #12
        # print(len(self.output_blocks)) #12
