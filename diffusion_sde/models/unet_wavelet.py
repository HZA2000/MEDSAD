...
class Conditioning(nn.Module):
    def __init__(self, fmap_size, dim):
        super().__init__()
        self.wavelet_parser_attn_map_L = nn.Parameter(torch.ones(dim, fmap_size // 4, fmap_size // 4))
        self.wavelet_parser_attn_map_H1 = nn.Parameter(torch.ones(dim, 6, fmap_size // 2, fmap_size // 2, 2))
        self.wavelet_parser_attn_map_H2 = nn.Parameter(torch.ones(dim, 6, fmap_size // 4, fmap_size // 4, 2))
        self.wavelet_parser_attn_map_H3 = nn.Parameter(torch.ones(dim, 6, fmap_size // 8, fmap_size // 8, 2))
    ...
    def forward(self, x):
        ...
        Yl = Yl * self.wavelet_parser_attn_map_L
        Yh1 = Yh1 * self.wavelet_parser_attn_map_H1
        Yh2 = Yh2 * self.wavelet_parser_attn_map_H2
        Yh3 = Yh3 * self.wavelet_parser_attn_map_H3

        Yh = tuple([Yh1, Yh2, Yh3])
